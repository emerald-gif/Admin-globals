...
async function sendNotification() {
  ...
  try {
    ...
    // Send email to all users (throttled to avoid rate limits)
...
usersSnapshot.forEach((doc, index) => {
  ...
  if (userEmail && userEmail.trim() !== "") {
    setTimeout(() => {
      sendEmailToUser(userEmail, title, message);
    }, index * 1500); // Delay each email by 1.5s to avoid limit
  } else {
    console.warn(`User ${doc.id} has no email, skipped.`);
  }
});
...
// Email sending logic
async function sendEmailToUser(email, subject, body) {
  try {
    await emailjs.send("service_5m7hr4q", "template_4nr76pb", {
      to_email: email,
      subject: subject,
      body: body
    });
