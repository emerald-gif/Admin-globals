...
function updateBillTabs() {
  ...
  if (ttl) ttl.textContent = (BILL_TYPE_LABELS[currentBillType]||currentBillType) + ' Requests';
}

function updateBillSubtabs() {
  ['pending','successful','failed'].forEach(s => {
    const el = document.getElementById(`subtab-${s}`); if (!el) return;
...
function loadBillsAdmin() {
  ...
  billsUnsub = db.collection('bill_submissions').orderBy('createdAt','desc').limit(100)
    .onSnapshot(snap => {
      container.innerHTML = '';
      if (snap.empty) {
      ...
      Promise.all(snap.docs.map(async doc => {
        const data = doc.data();
        const raw  = (data.type||'').toLowerCase();
        ...
        try {
          const tSnap = await db.collection('Transaction')
            .where('userId','==',data.userId).where('amount','==',data.amount)
        ...
        const badge = currentBillStatus==='successful'
        ...
        if (!container.hasChildNodes()||!container.querySelector('.rounded-2xl')) {
          container.innerHTML = `<div class="text-center py-16 text-gray-400">📭 No ${BILL_TYPE_LABELS[currentBillType]||currentBillType} ${currentBillStatus} requests</div>`;
        }
      });
    });
...
async function reviewBill(billId, userId, amount, approve, btnEl) {
  try {
    btnEl.disabled = true; btnEl.innerText = approve ? 'Approving...' : 'Rejecting...';
    ...
    if (!tSnap.empty) await tSnap.docs[0].ref.update({ status: approve ? 'successful' : 'failed' });
    if (!approve) {
      await db.runTransaction(async tx => {
        const u = await tx.get(userRef);
        tx.update(userRef, { balance: (Number(u.data().balance||0)) + Number(amount) });
      });
    }
    ...
    console.error(err); alert('⚠️ Failed: ' + err.message);
    btnEl.disabled = false; btnEl.innerText = approve ? '✅ Approve' : '❌ Reject';
  }
}
...
window.onAdminReady = function () {
  ...
  // Lazy-load new tabs on first nav click
  const tabActions = {
...
(function SurveyAdminModule(){
  ...
  async function surveyAdminLoad(){
    ...
    try{
      const [txSnap,usersSnap]=await Promise.all([
        db.collection('survey_transactions').orderBy('createdAt','desc').limit(300).get(),
        db.collection('users').where('surveysCompleted','>',0).get(),
      ]);
      _allTx=txSnap.docs.map(d=>({id:d.id,...d.data()}));
      let usd=0,naira=0,done=0;
      _allTx.forEach(d=>{
        if(['earn','credit'].includes(d.type)){usd+=Number(d.usdAmount||d.amount||0);done++;}
        if(['withdraw','withdrawal'].includes(d.type)){naira+=Number(d.nairaAmount||d.amount||0);}
      });
      _s('survey-total-usd','$'+usd.toFixed(2));
      _s('survey-total-naira',money(naira));
      _s('survey-earners-count',usersSnap.size);
      _s('survey-completed-count',done);
      surveyFilterType(_filter);
    }catch(e){console.error('[Survey Admin]',e);}
  }
  ...
  function surveyFilterType(type){
    ...
    const rows=type==='all'?_allTx:_allTx.filter(d=>{
    ...
    if(!rows.length){tbody.innerHTML='<tr><td colspan="6" style="text-align:center;padding:24px;color:rgba(255,255,255,.3);">No transactions found</td></tr>';return;}
    tbody.innerHTML=rows.map(d=>{
      ...
      return`<tr style="border-bottom:1px solid rgba(255,255,255,.06);">
        ...
        <td style="padding:8px 10px;font-size:.72rem;opacity:.6;">${(d.userId||'—').slice(0,14)}</td>
        ...
        <td style="padding:8px 10px;opacity:.5;">${fmt(d.createdAt)}</td>
      </tr>`;
    }).join('');
  }
...
(function AiLogsAdminModule() {
  ...
  let _counts     = { chat: '—', search: '—', writer: '—', insight: '—' };
  ...
  // Build the table for a given tab's data
  function _renderTable(tab, docs) {
    const wrap = document.getElementById('ai-logs-table-wrap');
    if (!wrap) return;

    if (!docs.length) {
      wrap.innerHTML = '<div style="color:rgba(255,255,255,.3);text-align:center;padding:40px;">No logs yet for this feature.</div>';
      return;
    }
    ...
    if (tab === 'chat') {
      headers = `<th>UID</th><th>Message</th><th>Reply (snippet)</th><th>Model</th><th>Has Image</th><th>Date</th>`;
      rowFn = d => `
        <td style="font-size:.7rem;opacity:.6;">${_esc((d.uid||'—').slice(0,16))}</td>
        <td style="max-width:220px;word-break:break-word;">${_esc(d.message||'—')}</td>
        <td style="max-width:200px;word-break:break-word;opacity:.7;">${_esc(d.replySnip||'—')}</td>
        <td style="font-size:.72rem;opacity:.6;">${_esc(d.model||'—')}</td>
        <td style="text-align:center;">${d.hasImage ? '🖼️' : '—'}</td>
        <td style="font-size:.72rem;opacity:.5;">${_fmt(d.timestamp)}</td>`;
    } else if (tab === 'search') {
      headers = `<th>UID</th><th>Query</th><th>Answer (snippet)</th><th>Navigations</th><th>Date</th>`;
      rowFn = d => `
        <td style="font-size:.7rem;opacity:.6;">${_esc((d.uid||'—').slice(0,16))}</td>
        <td style="max-width:220px;word-break:break-word;">${_esc(d.query||'—')}</td>
        <td style="max-width:200px;word-break:break-word;opacity:.7;">${_esc(d.answerSnip||'—')}</td>
        <td style="font-size:.72rem;opacity:.6;">${_esc(d.suggestions||'—')}</td>
        <td style="font-size:.72rem;opacity:.5;">${_fmt(d.timestamp)}</td>`;
    } else if (tab === 'writer') {
      ...
      rowFn = d => `
        ...
        <td style="opacity:.7;">${_esc(d.subcategory||'—')}</td>
        <td style="max-width:160px;word-break:break-word;opacity:.6;">${_esc(d.hint||'—')}</td>
        <td style="max-width:200px;word-break:break-word;">${_esc(d.titleSnip||'—')}</td>
        <td style="font-size:.72rem;opacity:.5;">${_fmt(d.timestamp)}</td>`;
    } else {
      // insight
      headers = `<th>UID</th><th>Category</th><th>Subcategory</th><th>Workers</th><th>Pay</th><th>Score</th><th>Verdict</th><th>Date</th>`;
      rowFn = d => {
        const scoreColor = d.score >= 80 ? '#34d399' : d.score >= 60 ? '#fbbf24' : '#f87171';
        return `
          <td style="font-size:.7rem;opacity:.6;">${_esc((d.uid||'—').slice(0,16))}</td>
          <td>${_esc(d.category||'—')}</td>
          <td style="opacity:.7;">${_esc(d.subcategory||'—')}</td>
          <td style="text-align:center;">${d.numWorkers||0}</td>
          <td>₦${Number(d.workerPay||0).toLocaleString()}</td>
          <td style="font-weight:800;color:${scoreColor};">${d.score||0}</td>
          <td style="max-width:200px;word-break:break-word;opacity:.7;">${_esc(d.verdict||'—')}</td>
          <td style="font-size:.72rem;opacity:.5;">${_fmt(d.timestamp)}</td>`;
      };
    }

    wrap.innerHTML = `
      <table style="width:100%;border-collapse:collapse;font-size:.8rem;min-width:700px;">
        <thead>
          <tr style="color:rgba(255,255,255,.45);text-align:left;border-bottom:1px solid rgba(255,255,255,.08);position:sticky;top:0;background:#0b1635;">
            ${headers.split('</th>').filter(Boolean).map(h =>
              `${h}</th>`).join('').replace(/<th>/g, '<th style="padding:8px 12px;font-weight:600;">') }
          </tr>
        </thead>
        <tbody>
          ${docs.map(d => `<tr style="border-bottom:1px solid rgba(255,255,255,.04);">${rowFn(d)}</tr>`)
            .join('')
            .replace(/<td/g, '<td style="padding:8px 12px;vertical-align:top;" ')}
        </tbody>
      </table>`;
  }
  ...
  async function _loadTab(tab) {
    ...
    try {
      const snap = await db.collection(COLLECTIONS[tab])
        .orderBy('timestamp', 'desc')
        .limit(150)
        .get();
      const docs = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      _counts[tab] = snap.size;
      _s(STAT_IDS[tab], snap.size);
      _renderTable(tab, docs);
    } catch(e) {
      console.error('[AI Logs Admin]', tab, e);
  ...
  async function aiLogsLoad() {
    // Load summary counts for all 4 tabs in parallel (lightweight — just .get() sizes)
    const keys = Object.keys(COLLECTIONS);
    await Promise.all(keys.map(async k => {
      try {
        const snap = await db.collection(COLLECTIONS[k]).orderBy('timestamp','desc').limit(1).get();
        // We can't get exact count cheaply, so load current tab fully
        _s(STAT_IDS[k], '…');
      } catch(e) { _s(STAT_IDS[k], 'err'); }
    }));
    // Load the active tab fully (gives real count)
    await _loadTab(_currentTab);
  }

  function aiLogsSwitch(tab) {
    _loadTab(tab);
  }
