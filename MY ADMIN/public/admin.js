...
async function fetchStats() {
  try {
    const [usersSnap, tasksSnap, withdrawalSnap, storesSnap, ludoTxSnap, billsSnap] = await Promise.all([
    ...
    // Banner chips
    const _s = (id, v) => { const el = document.getElementById(id); if (el) el.innerText = v; };
    _s('user-count',      usersSnap.size);
    _s('task-count',      tasksSnap.size);
    _s('withdrawal-count',withdrawalSnap.size);

    // Stat cards
    _s('user-count-card',      usersSnap.size);
    _s('task-count-card',      tasksSnap.size);
    _s('withdrawal-count-card',withdrawalSnap.size);

    // Premium count
    const premiumCount = usersSnap.docs.filter(d => d.data().is_Premium === true).length;
    _s('premium-count-overview', premiumCount);

    // Ludo payouts
    let ludoPayout = 0;
    ludoTxSnap.docs.forEach(d => { ludoPayout += Number(d.data().amount || 0); });
    _s('ludo-payout-overview', '₦' + ludoPayout.toLocaleString());

    // Store count
    _s('store-count-overview', storesSnap.size);

    // VTU transactions
    _s('vtu-count-overview', billsSnap.size);
  } catch(e) {
...
// ═══════════════════════════════════════════════════════════════════════
//  VTU BILLS MODULE  —  airtime | data | electricity | tv | betting | education
// ═══════════════════════════════════════════════════════════════════════
let currentBillType   = 'airtime'; // airtime|data|electricity|tv|betting|education
...
const BILL_TYPES   = ['airtime','data','electricity','tv','betting','education'];
...
function updateBillTabs() {
  BILL_TYPES.forEach((t, i) => {
    const el = document.getElementById(BILL_TAB_IDS[i]);
    if (!el) return;
  ...
  const titleEl = document.getElementById('bills-section-title');
  if (titleEl) titleEl.textContent = (BILL_TYPE_LABELS[currentBillType] || currentBillType) + ' Requests';
}
...
function updateBillSubtabs() {
  ['pending','successful','failed'].forEach(s => {
    ...
    if (!el) return;
    el.className = s === currentBillStatus
...
// Build extra detail rows for different service types
function _billExtraFields(data) {
  ...
  if (t === 'betting') {
    return `
      ${data.customerId    ? `<p><b>🆔 Customer ID:</b> ${data.customerId}</p>` : ''}
      ${data.customerName  ? `<p><b>👤 Name:</b> ${data.customerName}</p>` : ''}
  ...
  billsUnsub = db.collection('bill_submissions')
    ...
    .onSnapshot(snap => {
      ...
      Promise.all(snap.docs.map(async doc => {
        const data = doc.data();
        // Normalise type — electricity/tv/betting/education stored literally; airtime=default
        const rawType = (data.type || '').toLowerCase();
        const docType = ['electricity','tv','betting','education','data'].includes(rawType) ? rawType : 'airtime';
        if (docType !== currentBillType) return null;

        // Fetch linked transaction status
        let transStatus = 'processing';
        try {
          const tSnap = await db.collection('Transaction')
            .where('userId','==', data.userId)
            .where('amount','==', data.amount)
            .orderBy('timestamp','desc').limit(1).get();
          if (!tSnap.empty) transStatus = tSnap.docs[0].data().status || 'processing';
        } catch(e) { /* ignore */ }
        ...
        const badge =
          currentBillStatus === 'successful' ? '<span class="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">Successful</span>' :
          currentBillStatus === 'failed'     ? '<span class="px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">Failed</span>'       :
        ...
        card.innerHTML = `
          <div>
            ...
            <h3 class="text-lg font-bold text-gray-800 mb-2">₦${Number(data.amount||0).toLocaleString()}</h3>
            <div class="space-y-1 text-sm text-gray-600">
              ${_billExtraFields(data)}
              <p><b>👤 User ID:</b> <span class="font-mono text-xs">${data.userId || 'N/A'}</span></p>
              <p><b>🕒 Date:</b> ${data.createdAt?.toDate().toLocaleString() || 'N/A'}</p>
              ${data.requestId ? `<p><b>🔑 Ref:</b> <span class="font-mono text-xs">${data.requestId}</span></p>` : ''}
            </div>
          </div>
          ${!data.processed && currentBillStatus === 'pending' ? `
            <div class="flex gap-3 mt-4">
              <button onclick="reviewBill('${doc.id}','${data.userId}',${data.amount},true,this)"
                class="flex-1 px-3 py-2 rounded-lg bg-green-600 text-white text-sm font-medium hover:bg-green-700 transition">✅ Approve</button>
              <button onclick="reviewBill('${doc.id}','${data.userId}',${data.amount},false,this)"
                class="flex-1 px-3 py-2 rounded-lg bg-red-600 text-white text-sm font-medium hover:bg-red-700 transition">❌ Reject</button>
            </div>` : ''}
        `;
        return card;
      })).then(cards => {
        ...
        if (!container.hasChildNodes() || container.innerHTML === '') {
          container.innerHTML = `<div class="text-center py-16 text-gray-400">📭 No ${BILL_TYPE_LABELS[currentBillType]||currentBillType} ${currentBillStatus} requests</div>`;
        }
      });
    });
}
...
async function reviewBill(billId, userId, amount, approve, btnEl) {
  try {
    ...
    const transSnap = await db.collection('Transaction')
      .where('userId','==',userId).where('amount','==',amount)
      .where('status','==','processing').limit(1).get();
    if (!transSnap.empty) {
      await transSnap.docs[0].ref.update({ status: approve ? 'successful' : 'failed' });
    }
    if (!approve) {
      await db.runTransaction(async tx => {
        const uDoc = await tx.get(userRef);
        const bal = Number(uDoc.data().balance || 0);
        tx.update(userRef, { balance: bal + Number(amount) });
      });
    }
    const card = document.getElementById(`bill-${billId}`);
    if (card) card.remove();
    switchBillStatus(approve ? 'successful' : 'failed');
    alert(approve ? '✅ Approved!' : '❌ Rejected & refunded!');
  } catch(err) {
    console.error('reviewBill error:', err);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Wire new tab load-on-click for Ludo / Survey / EduStore
  // These lazy-load on first click so they don't slow down initial page load
  (function wireNewTabs() {
    ...
    document.querySelectorAll('.nav-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        // detect which tab this button opens by reading onclick attr
        const match = (btn.getAttribute('onclick') || '').match(/switchTab\('([^']+)'/);
        if (match && tabActions[match[1]]) tabActions[match[1]]();
      });
    });
  })();
...
(function LudoAdminModule() {
  ...
  function _money(n)  { return '₦' + Number(n || 0).toLocaleString(); }
  function _fmt(ts)   {
    if (!ts) return '—';
  ...
  async function ludoAdminLoad() {
    _s('ludo-active-rooms',    '…');
    _s('ludo-bet-rooms-count', '…');
    _s('ludo-total-payouts',   '…');
    _s('ludo-win-count',       '…');
    ...
    try {
      ...
      // Active rooms = those not finished
      const activeRooms = roomsSnap.docs.filter(d => d.data().status !== 'finished').length;
      _s('ludo-active-rooms',    activeRooms);
      _s('ludo-bet-rooms-count', betSnap.size);

      // Payouts
      let total = 0;
      txSnap.docs.forEach(d => { total += Number(d.data().amount || 0); });
      _s('ludo-total-payouts', _money(total));
      _s('ludo-win-count',     txSnap.size);

      // Also update overview card
      const ovEl = document.getElementById('ludo-payout-overview');
      if (ovEl) ovEl.textContent = _money(total);

      _renderBetRooms(betSnap.docs);
      _renderTxList(txSnap.docs);

    } catch(e) {
      console.error('[Ludo Admin]', e);
    }
  }

  function _renderBetRooms(docs) {
    const el = document.getElementById('ludo-bet-rooms-list');
    if (!el) return;
    if (!docs.length) { el.innerHTML = '<div style="color:rgba(255,255,255,0.3);text-align:center;padding:24px;">No bet rooms yet</div>'; return; }
    el.innerHTML = docs.slice(0,30).map(doc => {
      const d = doc.data();
      const statusColor = d.status === 'finished' ? '#34d399' : d.status === 'playing' ? '#f97316' : '#60a5fa';
      return `<div style="background:rgba(255,255,255,0.05);border-radius:10px;padding:12px;margin-bottom:8px;font-size:0.8rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
          <span style="font-weight:700;font-size:0.85rem;">${doc.id.slice(0,12)}…</span>
          <span style="color:${statusColor};font-weight:600;font-size:0.75rem;text-transform:uppercase;">${d.status || 'unknown'}</span>
        </div>
        <div style="color:rgba(255,255,255,0.55);display:grid;grid-template-columns:1fr 1fr;gap:4px;">
          <span>Stake: <b style="color:#f97316">${_money(d.stake)}</b></span>
          <span>Win: <b style="color:#34d399">${_money(d.winAmount)}</b></span>
          <span style="grid-column:1/-1;font-size:0.72rem;margin-top:2px;opacity:0.6;">P1: ${(d.p1||'').slice(0,10)} · P2: ${(d.p2||'—').slice(0,10)}</span>
          ${d.winner ? `<span style="grid-column:1/-1;color:#fbbf24;font-size:0.75rem;">🏆 Winner: ${d.winner.slice(0,12)}</span>` : ''}
        </div>
      </div>`;
    }).join('');
  }

  function _renderTxList(docs) {
    const el = document.getElementById('ludo-tx-list');
    if (!el) return;
    if (!docs.length) { el.innerHTML = '<div style="color:rgba(255,255,255,0.3);text-align:center;padding:24px;">No transactions yet</div>'; return; }
    el.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:0.78rem;">
      <thead><tr style="color:rgba(255,255,255,0.4);text-align:left;border-bottom:1px solid rgba(255,255,255,0.08);">
        <th style="padding:6px 8px;">Type</th><th style="padding:6px 8px;">Amount</th>
        <th style="padding:6px 8px;">UID</th><th style="padding:6px 8px;">Date</th>
      </tr></thead>
      <tbody>${docs.map(doc => {
        const d = doc.data();
        const color = d.type === 'ludo_win' ? '#34d399' : '#f97316';
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);color:rgba(255,255,255,0.8);">
          <td style="padding:6px 8px;color:${color};font-weight:600;">${d.type || '—'}</td>
          <td style="padding:6px 8px;font-weight:700;">${_money(d.amount)}</td>
          <td style="padding:6px 8px;font-size:0.7rem;opacity:0.6;">${(d.uid||'—').slice(0,14)}</td>
          <td style="padding:6px 8px;opacity:0.5;">${_fmt(d.timestamp)}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
  }
  ...
  function tryInit() {
    if (document.getElementById('ludo-active-rooms')) { ludoAdminLoad(); return; }
    if (document.readyState !== 'complete') window.addEventListener('DOMContentLoaded', () => { if(document.getElementById('ludo-active-rooms')) ludoAdminLoad(); });
  }
  tryInit();
})();
...
(function SurveyAdminModule() {
  ...
  async function surveyAdminLoad() {
    _s('survey-total-usd',      '…');
    _s('survey-total-naira',    '…');
    _s('survey-earners-count',  '…');
    _s('survey-completed-count','…');

    try {
      // survey_transactions holds both earn + withdraw records
      const [txSnap, usersSnap] = await Promise.all([
        db.collection('survey_transactions').orderBy('createdAt','desc').limit(200).get(),
        db.collection('users').where('surveysCompleted','>',0).get(),
      ]);

      _allTx = txSnap.docs.map(d => ({ id: d.id, ...d.data() }));

      let totalUsd = 0, totalNaira = 0, totalCompleted = 0;
      _allTx.forEach(d => {
        if (d.type === 'earn' || d.type === 'credit') {
          totalUsd       += Number(d.usdAmount || d.amount || 0);
          totalCompleted += 1;
        }
        if (d.type === 'withdraw' || d.type === 'withdrawal') {
          totalNaira += Number(d.nairaAmount || d.amount || 0);
        }
      });

      _s('survey-total-usd',      '$' + totalUsd.toFixed(2));
      _s('survey-total-naira',    _money(totalNaira));
      _s('survey-earners-count',  usersSnap.size);
      _s('survey-completed-count',totalCompleted);

      surveyFilterType(_currentFilter);
  ...
  function surveyFilterType(type) {
    ...
    ['all','earn','withdraw'].forEach(t => {
      const el = document.getElementById(`survey-filter-${t}`);
      if (!el) return;
      el.style.background = t === type ? '#14b8a6' : 'rgba(255,255,255,0.08)';
      el.style.color       = t === type ? '#fff'    : 'rgba(255,255,255,0.7)';
    });
    const filtered = type === 'all' ? _allTx : _allTx.filter(d => {
      if (type === 'earn')     return ['earn','credit'].includes(d.type);
      if (type === 'withdraw') return ['withdraw','withdrawal'].includes(d.type);
      return true;
    });
    _renderTxTable(filtered);
  }

  function _renderTxTable(rows) {
    const tbody = document.getElementById('survey-tx-body');
    if (!tbody) return;
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:24px;color:rgba(255,255,255,0.3);">No transactions found</td></tr>';
      return;
    }
    tbody.innerHTML = rows.map(d => {
      const isEarn = ['earn','credit'].includes(d.type);
      const color  = isEarn ? '#34d399' : '#f87171';
      const label  = isEarn ? '💰 Earn' : '💸 Withdraw';
      return `<tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
        <td style="padding:8px 10px;color:${color};font-weight:600;">${label}</td>
        <td style="padding:8px 10px;font-size:0.72rem;opacity:0.6;">${(d.userId||'—').slice(0,14)}</td>
        <td style="padding:8px 10px;font-weight:700;">$${Number(d.usdAmount||d.amount||0).toFixed(2)}</td>
        <td style="padding:8px 10px;">${d.nairaAmount ? _money(d.nairaAmount) : '—'}</td>
        <td style="padding:8px 10px;opacity:0.7;">${d.providerName||d.provider||'—'}</td>
        <td style="padding:8px 10px;opacity:0.5;">${_fmt(d.createdAt)}</td>
      </tr>`;
    }).join('');
  }
  ...
  function tryInit() {
    if (document.getElementById('survey-total-usd')) { surveyAdminLoad(); return; }
    if (document.readyState !== 'complete') window.addEventListener('DOMContentLoaded', () => { if(document.getElementById('survey-total-usd')) surveyAdminLoad(); });
  }
  tryInit();
})();
...
(function EduStoreAdminModule() {
  ...
  async function edustoreAdminLoad() {
    ...
    try {
      ...
      txSnap.docs.forEach(d => {
        totalRev    += Number(d.data().merchantPrice   || 0);
        totalProfit += Number(d.data().merchantProfit  || 0);
      });

      _s('edu-store-count',    storesSnap.size);
      _s('edu-tx-count',       txSnap.size);
      _s('edu-merchant-rev',   _money(totalRev));
      _s('edu-merchant-profit',_money(totalProfit));

      // Also update overview store count
      const ovEl = document.getElementById('store-count-overview');
      if (ovEl) ovEl.textContent = storesSnap.size;

      _renderStores(storesSnap.docs);
      _renderTx(txSnap.docs);

    } catch(e) {
      console.error('[EduStore Admin]', e);
    }
  }

  function _renderStores(docs) {
    const el = document.getElementById('edu-stores-list');
    if (!el) return;
    if (!docs.length) { el.innerHTML = '<div style="color:rgba(255,255,255,0.3);text-align:center;padding:24px;">No stores created yet</div>'; return; }
    el.innerHTML = docs.map(doc => {
      const d = doc.data();
      const services = Object.entries(d.services || {}).filter(([,v]) => v.enabled).map(([k]) => k.toUpperCase()).join(', ');
      const statusColor = d.status === 'active' ? '#34d399' : '#f87171';
      return `<div style="background:rgba(255,255,255,0.05);border-radius:10px;padding:14px;margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
          <div>
            <div style="font-weight:700;font-size:0.9rem;">${_esc(d.businessName || doc.id)}</div>
            <div style="font-size:0.72rem;opacity:0.5;margin-top:2px;">/${_esc(doc.id)}.globalstasks.name.ng</div>
          </div>
          <span style="color:${statusColor};font-size:0.72rem;font-weight:600;text-transform:uppercase;">${d.status||'active'}</span>
        </div>
        <div style="font-size:0.78rem;color:rgba(255,255,255,0.55);display:grid;grid-template-columns:1fr 1fr;gap:4px;">
          <span>💼 Services: <b style="color:#fb923c">${_esc(services||'—')}</b></span>
          <span>📦 Txns: <b style="color:#60a5fa">${d.totalTransactions||0}</b></span>
          <span>💰 Revenue: <b style="color:#34d399">${_money(d.totalRevenue)}</b></span>
          <span>📅 Created: <span style="opacity:0.6">${_fmt(d.createdAt)}</span></span>
        </div>
        <div style="font-size:0.7rem;opacity:0.4;margin-top:6px;">Merchant UID: ${(d.merchantUid||'—').slice(0,18)}</div>
      </div>`;
    }).join('');
  }

  function _renderTx(docs) {
    const el = document.getElementById('edu-tx-list');
    if (!el) return;
    if (!docs.length) { el.innerHTML = '<div style="color:rgba(255,255,255,0.3);text-align:center;padding:24px;">No transactions yet</div>'; return; }
    el.innerHTML = `<table style="width:100%;border-collapse:collapse;font-size:0.78rem;">
      <thead><tr style="color:rgba(255,255,255,0.4);text-align:left;border-bottom:1px solid rgba(255,255,255,0.08);">
      ...
      <tbody>${docs.map(doc => {
        const d = doc.data();
        const statusColor = d.status === 'delivered' ? '#34d399' : d.status === 'failed' ? '#f87171' : '#fbbf24';
        return `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);color:rgba(255,255,255,0.8);">
          <td style="padding:6px 8px;opacity:0.8;">${_esc(d.storeSlug||'—')}</td>
          <td style="padding:6px 8px;opacity:0.7;font-size:0.72rem;">${_esc(d.planName||d.serviceID||'—')}</td>
          <td style="padding:6px 8px;font-weight:700;">${_money(d.merchantPrice)}</td>
          <td style="padding:6px 8px;color:#34d399;font-weight:600;">${_money(d.merchantProfit)}</td>
          <td style="padding:6px 8px;color:${statusColor};font-weight:600;font-size:0.72rem;">${d.status||'—'}</td>
          <td style="padding:6px 8px;opacity:0.5;font-size:0.72rem;">${_fmt(d.timestamp)}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>`;
  }
  ...
  function tryInit() {
    if (document.getElementById('edu-store-count')) { edustoreAdminLoad(); return; }
    if (document.readyState !== 'complete') window.addEventListener('DOMContentLoaded', () => { if(document.getElementById('edu-store-count')) edustoreAdminLoad(); });
  }
  tryInit();
})();
